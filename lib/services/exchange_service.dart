...
// // class ExchangeService with ChangeNotifier {
// //   double _amount = 1.0;
// //   Currency _fromCurrency = Currency.popularCurrencies[0];
// //   Currency _toCurrency = Currency.popularCurrencies[1];
...
// //     try {
// //       final apiKey = dotenv.env['EXCHANGE_RATE_API_KEY'] ?? 'your_api_key_here';
...
// //         // Use sample rates for demo if API fails
// //         _useSampleRates();
// //       }
// //     } catch (e) {
...
// //     // Sample exchange rates (1 USD to other currencies)
// //     _exchangeRates = {
// //       'USD': 1.0,
// //       'EUR': 0.85,
// //       'GBP': 0.73,
...
class ExchangeService with ChangeNotifier {
  ...
  Future<void> fetchExchangeRates() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        final rates = _parseRates(data['conversion_rates']);
        _exchangeRates = rates;
        _lastUpdated = DateTime.now();
        _convert();
      } else {
        _errorMessage = 'Failed to fetch exchange rates. Using sample data.';
        _useSampleRates();
      }
    } catch (e) {
      _errorMessage = 'Network error: ${e.toString().split('\n').first}';
      _useSampleRates();
    } finally {
      _isLoading = false;
      notifyListeners();
  ...
  if (ratesData is Map) {
    ratesData.forEach((key, value) {
      if (value is int) {
        parsedRates[key.toString()] = value.toDouble();  // Convert int to double
      } else if (value is double) {
        parsedRates[key.toString()] = value;  // Keep as double
      } else if (value is String) {
        parsedRates[key.toString()] = double.tryParse(value) ?? 0.0;
      } else if (value is num) {  // Handle both int and double
  ...
  void _useSampleRates() {
    // Extended sample rates - all as doubles
    _exchangeRates = {
      'USD': 1.0,    'EUR': 0.85,   'GBP': 0.73,   'JPY': 110.0,
      'AUD': 1.35,   'CAD': 1.25,   'CHF': 0.92,   'CNY': 6.45,
      'INR': 74.0,   'SGD': 1.34,   'NZD': 1.42,   'KRW': 1180.0,
